...
public:
	...
	void initCamera(const glm::vec3& pos, float fov, float aspect, float nearClip, float farClip, const glm::vec3& lookAt)
	{
		this->pos = pos;
	...
	{
		pos = startPos;
		forward = glm::normalize(startForward);
	...
	void setLook(const glm::vec3& modelPos)
	{
		forward = glm::vec3(glm::normalize(modelPos - pos));
	}

	void setPos(const glm::vec3& modelPos)
	{
		pos = modelPos;
	}
